...
class ModelPredictiveController {
  public:
    ModelPredictiveController(size_t horizon, double dt) : horizon_(horizon), dt_(dt) {
        Q_ = {1.0, 1.0, 0.5, 0.1};
        R_ = {0.1, 0.1};
        dR_ = {0.01, 0.01};
        lf_ = 1.0;
    ...
    void setReference(const std::vector<Reference> &ref) { reference_traj_ = ref; }
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle) {
        ...
        for (const geometry_msgs::msg::Point &element : segments) {
            ...
            while (rclcpp::ok()) {
                ...
                // RCLCPP_INFO(this->get_logger(), "Linear: %f, Angular: %f, Distance: %f", nav_params[0],
                // nav_params[1], nav_params[2]);
