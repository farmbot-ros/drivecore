...
class Navigator : public rclcpp::Node {
  ...
  public:
    Navigator()
        ...
        this->action_server_ = rclcpp_action::create_server<TheAction>(
            this, "con/zeroturn",
        ...
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "loc/odom", 10, [this](const nav_msgs::msg::Odometry::SharedPtr msg) { current_pose_ = msg->pose.pose; });
  ...
  private:
    ...
    void check_system(diagnostic_updater::DiagnosticStatusWrapper &stat) {
        ...
        stat.add("Current Pose",
                 "x: " + std::to_string(current_pose_.position.x) + " y: " + std::to_string(current_pose_.position.y));
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle) {
        ...
        std::vector<geometry_msgs::msg::Point> segments =
            generateSegments(current_pose_.position, goal->segment.destination.pose.position);
        for (const geometry_msgs::msg::Point &element : segments) {
            target_pose_ = element;
            // RCLCPP_INFO(this->get_logger(), "Going to: %f, %f, currently at: %f, %f", target_pose_.x, target_pose_.y,
            ...
            while (rclcpp::ok()) {
                ...
                if (std::hypot(current_pose_.position.x - target_pose_.x, current_pose_.position.y - target_pose_.y) <
                    goal_threshold) {
                    break;
                }
    ...
    std::array<double, 3> get_nav_params(double angle_max = 3.14 / 4, double velocity_max = 1.0,
                                         double velocity_scale = 0.2, bool zeroturn = true, double heading_gain = 1.5,
        ...
        // RCLCPP_INFO(this->get_logger(), "Desired Heading: %f, Current Heading: %f", rad2deg(preheading),
        // rad2deg(orientation)); Calculate the heading difference and normalize it
    ...
    std::vector<geometry_msgs::msg::Point> generateSegments(const geometry_msgs::msg::Point &start,
                                                            const geometry_msgs::msg::Point &end, int n = 1) {
        std::vector<geometry_msgs::msg::Point> segments;
        if (n <= 1) {
